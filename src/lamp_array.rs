...
use crate::profile::LampArrayProfile;
use hidapi::HidApi;
...
fn selected(profile: &LampArrayProfile) -> Result<DeviceCandidate> {
    let vid = profile.vid;
    let pid = profile.pid;
    choose_candidate(discover_hidraw()?, profile).with_context(|| {
        format!(
...
fn choose_candidate(
    ...
    profile: &LampArrayProfile,
) -> Option<DeviceCandidate> {
    candidates.into_iter().find(|candidate| {
        candidate.lamp_array
            && profile.vid.is_none_or(|wanted| candidate.vid == wanted)
            && profile.pid.is_none_or(|wanted| candidate.pid == wanted)
    })
}
fn open(profile: &LampArrayProfile) -> Result<hidapi::HidDevice> {
    let candidate = selected(profile)?;
...
pub fn set_color(profile: &LampArrayProfile, rgb: [u8; 3]) -> Result<()> {
    let device = open(profile)?;
...
mod tests {
    ...
    #[test]
    fn selection_requires_both_profile_ids_and_lamp_array_usage() {
        let candidate = |path: &str, vid, pid, lamp_array| DeviceCandidate {
            path: path.into(),
            vid,
            pid,
            lamp_array,
        ...
        let candidates = vec![
            candidate("/dev/hidraw0", 0x0b05, 0x18f3, false),
            candidate("/dev/hidraw1", 0x0b05, 0x1234, true),
            candidate("/dev/hidraw2", 0x0b05, 0x18f3, true),
        ];
        let selected =
            choose_candidate(candidates, &crate::profile::DEFAULT_HARDWARE.lamp_array).unwrap();
        assert_eq!(selected.path, PathBuf::from("/dev/hidraw2"));
    }
    #[test]
